...
- Make the project perform the function referenced in the text snippet below
- To test this, you can use the .NET project located /Users/alanhape/Projects/libs/BenchmarkDotNet/BenchmarkDotNet.slnx (note this is .slnx, but in most cases this script will be used for .sln) and try searching for the definition of "RuntimeMoniker" (the definition is located /Users/alanhape/Projects/libs/BenchmarkDotNet/src/BenchmarkDotNet.Annotations/Jobs/RuntimeMoniker.cs:L5)

* you are not allowed to install any external dependencies (Nuget) that aren't referenced in the text snippet below
...
The output will show a Context (containing type). Use that context to decide which private method is relevant to your current task.”Why this solves the scoping problem
...
2. -name mode (the filter): For a symbol like DoWork, the script prints:
